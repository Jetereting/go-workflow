...
func (i *Identitylink) SaveTx(tx *gorm.DB) error {
	...
	// }
	i.Created = time.Now().Format("2006-01-02 15:04:05")
...
func FindParticipantByProcInstID(procInstID int) ([]*Identitylink, error) {
	...
	err := db.Select("id,user_id,user_name,step,comment,created").Where("proc_inst_id=? and type=?", procInstID, IdentityTypes[PARTICIPANT]).Order("id asc").Find(&datas).Error
	return datas, err
}

// FindUserFlow 查询用户参与的流程
func FindUserFlow(userId string, isLauncher bool) (result []int, err error) {
	var data []struct {
