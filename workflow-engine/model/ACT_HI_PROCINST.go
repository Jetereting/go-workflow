...
// 分页查询
func FindProcInsts(userID, procName, company string, groups, departments []string, isFinished bool, pageIndex, pageSize int) ([]*ProcInst, int, error) {
	var datas []*ProcInst
	var count int

	var sql = " company='" + company + "' "
	if isFinished {
	...
	// fmt.Println(sql)
	selectData := func(in chan<- error, wg *sync.WaitGroup) {
		go func() {
